...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('views/modal.rule.html',
    '<form class="form-horizontal form-modal" role="form" name="ruleForm"><div class="modal-header"><button type="button" class="close" data-ng-click="cancel()"><span aria-hidden="true">&times;</span> <span class="sr-only">Close</span></button><h4 class="modal-title" data-ng-bind="modalTitle"></h4></div><div class="modal-body"><div data-alert="" data-type="danger" data-ng-show="alert.show" class="animated fade-in"><i class="iconfont icon-danger"></i> <span data-ng-bind="alert.msg"></span></div><div class="form-group has-feedback" data-ng-class="{\'has-success\':ruleForm.city.$valid}"><label for="province" class="col-xs-2 control-label">省市名称</label><div class="col-xs-5"><select name="province" class="form-control" data-ng-model="province" data-ng-options="prov.name for prov in extraData" data-ng-change="formData.provinceId=province.aid; city=\'\'" required=""><option value="">-- 请选择 --</option></select><input name="provinceId" type="hidden" data-ng-model="formData.provinceId"></div><div class="col-xs-4"><select name="city" class="form-control" data-ng-model="city" data-ng-options="city.cityName for city in province.cities" data-ng-change="formData.cityId=city.cityId" required=""><option value="">-- 请选择 --</option></select><input name="cityId" type="hidden" data-ng-model="formData.cityId"></div></div><div class="form-group has-feedback" data-ng-class="{\'has-success\':ruleForm.startPrice.$valid, \'has-error\':ruleForm.startPrice.$invalid &amp;&amp; ruleForm.startPrice.$dirty}"><label for="startPrice" class="col-xs-2 control-label">起步价格</label><div class="col-xs-9"><input type="number" class="form-control" name="startPrice" data-ng-model="formData.startPrice" required="" placeholder=""><i class="glyphicon form-control-feedback" data-ng-show="ruleForm.startPrice.$dirty" data-ng-class="{\'glyphicon-ok\':ruleForm.startPrice.$valid, \'glyphicon-remove\':ruleForm.startPrice.$invalid}"></i></div></div><div class="form-group has-feedback" data-ng-class="{\'has-success\':ruleForm.extraPrice.$valid, \'has-error\':ruleForm.extraPrice.$invalid &amp;&amp; ruleForm.extraPrice.$dirty}"><label for="extraPrice" class="col-xs-2 control-label">加收金额</label><div class="col-xs-9"><input type="number" class="form-control" name="extraPrice" data-ng-model="formData.extraPrice" required="" placeholder=""><i class="glyphicon form-control-feedback" data-ng-show="ruleForm.extraPrice.$dirty" data-ng-class="{\'glyphicon-ok\':ruleForm.extraPrice.$valid, \'glyphicon-remove\':ruleForm.extraPrice.$invalid}"></i></div></div><div class="form-group has-feedback" data-ng-class="{\'has-success\':ruleForm.description.$valid, \'has-error\':ruleForm.description.$invalid &amp;&amp; ruleForm.description.$dirty}"><label for="description" class="col-xs-2 control-label">规则描述</label><div class="col-xs-9"><textarea class="form-control" name="description" rows="3" data-ng-model="formData.description"></textarea></div></div></div><div class="modal-footer"><button class="btn btn-primary" data-ng-disabled="ruleForm.$invalid" data-ng-click="confirm()"><i class="glyphicon glyphicon-ok"></i> 确认</button><button class="btn" data-ng-click="cancel()"><i class="glyphicon glyphicon-ban-circle"></i> 取消</button></div></form>');
}]);
})();
;(function(module) {
try {
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('views/order.html',
    '<div class="well"><form class="form-inline pull-right" role="form"><div class="form-group"><label class="sr-only">名称</label><input type="text" class="form-control" name="demo1" data-ng-model="search.name" placeholder="名称"></div><div class="form-group"><label class="sr-only">账号</label><input type="text" class="form-control" name="demo2" data-ng-model="search.account" placeholder="账号"></div><div class="form-group"><button type="submit" class="btn btn-primary"><span class="glyphicon glyphicon-search"></span> 搜素</button></div></form><button class="btn btn-primary" data-ng-click="syncOrder()" data-ng-class="{\'sync-ing\':rotate}"><i class="glyphicon glyphicon-refresh"></i> <span data-ng-bind="syncStatus"></span></button><div class="clearfix"></div></div><table class="table table-bordered table-striped table-dds"><thead><tr><td>订单号</td><td>客户信息</td><td>司机信息</td><td>里程</td><td>费用</td><td>目的地</td><td>创建时间</td><td>起始时间</td><td>服务时段</td><td>状态</td></tr></thead><tbody><tr data-ng-repeat="b in bill"><td data-ng-bind="b.billNo"></td><td><span data-ng-bind="b.cName"></span><br><span data-ng-bind="b.cMobile"></span></td><td><span data-ng-bind="b.dName"></span><br><span data-ng-bind="b.dMobile"></span></td><td><span data-ng-bind="b.distance"></span> km</td><td>￥ <span data-ng-bind="b.charge"></span></td><td data-ng-bind="b.destination"></td><td data-ng-bind="b.createTime"></td><td><span data-ng-bind="b.startTime | date:\'hh:mm:ss\'"></span><br><span data-ng-bind="b.endTime | date:\'hh:mm:ss\'"></span></td><td><span data-ng-bind="b.hours"></span> 小时</td><td><i class="glyphicon" data-ng-class="{\'glyphicon-ok-sign text-success\':b.status==0, \'glyphicon-remove-sign text-danger\':b.status==1, \'glyphicon-question-sign\':b.status>1}"></i></td></tr></tbody></table><div class="row" data-ng-show="showPagination"><div class="col-xs-10"><div data-pagination="" data-total-items="total" data-ng-model="pageNo" data-max-size="maxPageSize" class="pagination-sm" data-num-pages="numPages" data-ng-change="changePage()" data-rotate="false" data-boundary-links="true" data-previous-text="Prev"></div></div><div class="col-xs-2 text-right pagination-tip">Page: {{pageNo}} / {{numPages}}</div></div>');
}]);
})();
;(function(module) {
try {
